...
function stop_audio(sound) {
  if (!sound.ended) {
    sound.pause();
    sound.currentTime = 0;  // 再生位置を0秒にする
  }
}
var warn = new Audio();
function warn_beep() {
  warn = new Audio("./media/warning.wav");
  //console.log(if_stop);
  warn.play()
}
function stop_warn() {
...
function judge_beep(if_right) {
  var judge = new Audio();
  switch (if_right) {
    case "right":
      judge.src = "./media/correct.wav"
      break;
    case "wrong":
      judge.src = "./media/wrong.wav"
      break;
    case "time_up":
      judge.src = "./media/chinchin.wav"
      break;
    default:
      judge.src = "./media/chinn.wav"
  }
  judge.play();
}

function fade_audio(sound) {
  //sound.volume = 0.7;
  var fade_sound = setInterval(function () {
    if (sound.volume > 0.1 ) {
      sound.volume -= 0.02
    }
  }, 100);
  if (sound.volume <= 0.1) {
    clearInterval(fade_sound);
  }
}
